...
inline bool _AtlVerifyStackAvailable(SIZE_T Size)
{
	...
	SIZE_T nActualSize=Size + _ATL_STACK_MARGIN;
	if(nActualSize<Size)
	{
		return false;
	}
...
public :
        ...
        {
				SIZE_T nActualSize=nRequestedSize + sizeof(CAtlSafeAllocBufferNode);
				if(nActualSize<nRequestedSize)
				{
					return NULL;
				}
                CAtlSafeAllocBufferNode *p = (CAtlSafeAllocBufferNode*)Allocator::Allocate(nActualSize);
                if (p == NULL)
                        return NULL;
...
#pragma warning(push)
#pragma warning(disable:4995) // lstrcpy is deprecated
