...
// Implementation
	...
	STDMETHOD(SetLinkSource)(DWORD, __in_ecount_z(lenFileName+1) LPTSTR, ULONG lenFileName, ULONG*, BOOL);
	STDMETHOD(GetLinkSource)(DWORD, __deref_out LPTSTR*, __out_opt ULONG*, __deref_out_opt LPTSTR*,
		__deref_out_opt LPTSTR*, __out_opt BOOL*, __out_opt BOOL*);
...
public:
	...
	BEGIN_INTERFACE_PART(OleUIObjInfo, IOleUIObjInfo)
		...
		STDMETHOD(GetObjectInfo) (THIS_ DWORD dwObject,
			__out_opt DWORD* lpdwObjSize, __deref_out_z_opt LPTSTR* lplpszLabel,
			__deref_out_z_opt LPTSTR* lplpszType, __deref_out_z_opt LPTSTR* lplpszShortType,
			__deref_out_z_opt LPTSTR* lplpszLocation);
