...
public:
        ...
        HRESULT  RegisterBuffer(LPCTSTR szReg, BOOL bRegister);
...
protected:
...
#ifdef OLD_ATL_CRITSEC_CODE
#ifdef ATL_STATREG_MIXED_MODEL_REGISTRATION
    ...
    // initialization sizes for the CRegObject::ctor.  
    // since registration doesn't happen that often, we're just going to take
    // the microscopic perf hit of always assuming multithread and really grabbing a real critsec.  the alternative
    // would be to templatize all the CRegXXX classes with a threading model parameter.  
    // but, its not worth doing that much work for such a low use frequency component.
...
#pragma warning(push)
#pragma warning(disable:4995) // lstrcat is deprecated
    ...
    while (*m_pchCur != NULL) // look for end
    {
        if (*m_pchCur == _T('%')) {
            ...
            if (*m_pchCur == _T('%')) {
                ...
                LPCTSTR lpszNext = StrChr(m_pchCur, _T('%'));
                if (lpszNext == NULL) {
...
inline HRESULT CRegParser::RegisterBuffer(LPCTSTR szBuffer, BOOL bRegister)
{
        TCHAR*  szToken;
        HRESULT hr = S_OK;
        ...
        while (NULL != *m_pchCur)
        {
                ...
                if (bRegister)
                {
                        LPCTSTR szRegAtRegister = m_pchCur;
