...
#define _ATL_PERSIST_QI_SUPPORT
#endif
...
#ifdef ATL_COM_THREADSAFE_ENUMERATORS
#define ATL_LOCK() \
...
inline HRESULT WINAPI AtlReportError2(const CLSID& clsid, UINT nID, const IID& iid,
	HRESULT hRes, HINSTANCE hInst, va_list* Arguments)
{
	return AtlSetErrorInfo2(clsid, nID, 0, NULL, iid, hRes, hInst, Arguments);
}

inline HRESULT WINAPI AtlReportError2(const CLSID& clsid, UINT nID, DWORD dwHelpID,
	LPCOLESTR lpszHelpFile, const IID& iid, HRESULT hRes, HINSTANCE hInst, va_list* Arguments)
{
	return AtlSetErrorInfo2(clsid, nID, dwHelpID,
		lpszHelpFile, iid, hRes, hInst, Arguments);
}
...
#define PROP_ENTRY(szDesc, dispid, clsid) \
                {OLESTR(szDesc), dispid, &clsid, &IID_IDispatch, 0, 0, VT_EMPTY},
...
#define CHAIN_PROP_MAP(theChainClass) \
        {NULL, 0, reinterpret_cast<CLSID*>(&theChainClass::GetPropertyMap), \
            &__uuidof(IChainPropMapFlagDummy), offsetofclass(theChainClass, _PropMapClass), 0, VT_NULL},
...
#ifdef _ATL_PERSIST_QI_SUPPORT
#define PROP_DATA_QI_ENTRY(szDesc, member, iid) \
		{OLESTR(szDesc), 0, &CLSID_NULL, &iid, offsetof(_PropMapClass, member), sizeof(((_PropMapClass*)0)->member), VT_UNKNOWN},
#endif
...
#else
ATLINLINE ATLAPI AtlIPersistStreamInit_Load2(LPSTREAM pStm, ATL_PROPMAP_ENTRY* pMap, void* pThis, IUnknown* pUnk, bool fVersionCheck)
{
        ATLASSERT(pMap != NULL);
        if (pStm == NULL || pMap == NULL || pThis == NULL || pUnk == NULL)
#ifdef ATL_COM_COMPLIANT_HRESULTS
                return E_POINTER;
#else
                return E_INVALIDARG;
#endif
        ...
        HRESULT hr = S_OK;
        if (fVersionCheck) {  // allow us to skip this on map chaining(saves space, only ver header at most derived)
                DWORD dwVer;
        ...
        {
                if (pMap[i].piidDispatch && *pMap[i].piidDispatch == __uuidof(IChainPropMapFlagDummy)) {
                    ...
                    hr = AtlIPersistStreamInit_Load2(pStm, p,
                        reinterpret_cast<void*>(pMap[i].dwOffsetData + (DWORD_PTR)pThis), 
                        pUnk, false /* no ver check in chained bases(only in most derived) */);
                    if (FAILED(hr)) {
                        ATLTRACE2(atlTraceCOM, 0, _T("Failed to Chain Property Map in Stream Load idx = %d\n"), i);
                        return hr;
                    }
                    continue;
                }
                ...
                void* pData = reinterpret_cast<void*>(pMap[i].dwOffsetData + (DWORD_PTR)pThis);

                if (pMap[i].dwSizeData != 0) {
                    switch (pMap[i].vt) {
                    // directly handle all the primitive types
                    case VT_UI1:
                    case VT_I1:
                    case VT_BOOL:
                    case VT_I2:
                    ...
                    // fall through and let CComVariant handle everything else.
                    // i'm being pedantic and listing all the types we expect to handle
                    // although i could just leave it to fall into default
                    case VT_UNKNOWN: {
                        LPUNKNOWN punk = *reinterpret_cast<LPUNKNOWN *>(pData);
                        if (punk) {
                            punk->Release();
                        }
                        // clear value we're about to copy on top of all fall through to detaching memcpy
                    } break;
                    case VT_DISPATCH: {
                        LPDISPATCH pdisp = *reinterpret_cast<LPDISPATCH *>(pData);
                        if (pdisp) {
                            pdisp->Release();
                        }
                        // clear value we're about to copy on top of all fall through to detaching memcpy
                    ...
                    case VT_ARRAY | VT_UI1: {
                        SAFEARRAY* psa = *reinterpret_cast<SAFEARRAY**>(pData);
                        if (psa) {
                    ...
                    default:
                        // fall-through and let CComVariant handle everything now
                        break;
                    }
                }
...
#ifdef _ATL_PERSIST_QI_SUPPORT
                if (pMap[i].vt == VT_UNKNOWN) {
                    // PROP_DATA_QI_ENTRY
                    ATLASSERT(var.vt == VT_UNKNOWN || var.vt == VT_DISPATCH);  // var should always match
                    if ((var.vt == VT_UNKNOWN || var.vt == VT_DISPATCH) &&
                        var.punkVal != NULL &&
                        pMap[i].piidDispatch) {
                            var.punkVal->QueryInterface(*(pMap[i].piidDispatch), (void **) pData);
                            var.vt = VT_EMPTY;
                            var.punkVal = NULL;
                            continue;
                    }
...
#endif		
                if (pMap[i].dwSizeData != 0) {
                    memcpy(pData, &var.bVal, pMap[i].dwSizeData);
                    // in this case, we've directly detached the contents of this variant into our object.
                    // so, don't let the variant get destroyed normally since that will free/release stuff 
...
ATLINLINE ATLAPI AtlIPersistStreamInit_Load(LPSTREAM pStm, ATL_PROPMAP_ENTRY* pMap, void* pThis, IUnknown* pUnk) {
    return AtlIPersistStreamInit_Load2(pStm, pMap, pThis, pUnk, true);
}
#endif //_ATL_DLL
...
#else
ATLINLINE ATLAPI AtlIPersistStreamInit_Save2(LPSTREAM pStm,
        BOOL fClearDirty, ATL_PROPMAP_ENTRY* pMap,
        void* pThis, IUnknown* pUnk, bool fVersionCheck)
{
        ATLASSERT(pMap != NULL);
        if (pStm == NULL || pMap == NULL || pThis == NULL || pUnk == NULL)
#ifdef ATL_COM_COMPLIANT_HRESULTS
        ...
        for (int i = 0; pMap[i].pclsidPropPage != NULL; i++)
        {
                if (pMap[i].piidDispatch && *pMap[i].piidDispatch == __uuidof(IChainPropMapFlagDummy)) {
                    ...
                    hr = AtlIPersistStreamInit_Save2(pStm, fClearDirty, p, 
                        reinterpret_cast<void*>(pMap[i].dwOffsetData + (DWORD_PTR)pThis), 
                        pUnk, false /* no ver check in chained bases(only in most derived) */);
                    if (FAILED(hr)) {
                        ATLTRACE2(atlTraceCOM, 0, _T("Failed to Chain Property Map in Stream Save = %d\n"), i);
                        return hr;
                    }
                    continue;
                }
                ...
                if (pMap[i].dwSizeData != 0)
                {
                    void* pData = (void*) (pMap[i].dwOffsetData + (DWORD_PTR)pThis);
                        ...
                        var.vt = pMap[i].vt;
                        switch (var.vt) {
                        // direct write all the primitive types
                        case VT_UI1:
                        case VT_I1:
                        case VT_BOOL:
                        case VT_I2:
                        ...
                        case VT_CLSID: {
#if 0
                                // don't write the tag, we know the type and it breaks back compat
                                // with existing persisted instances of the ocx
                                hr = pStm->Write(&var.vt, sizeof(VARTYPE), NULL);
                                if (FAILED(hr)) {
                                                return hr;
                                }
#endif
                        ...
                        case VT_BSTR_BLOB:
                        default:
                            var.byref = *(reinterpret_cast<LPVOID*>(pData));
                            hr = var.WriteToStream(pStm);
                            if (FAILED(hr)) {
                                return hr;
                            }
                            // in this case, this isn't a real variant. its actually pointing into our object
                            // don't let it get cleared.  it will try to free data that didn't come
                            // from the heap. and, in the com ptr case, we didn't addref and don't want to release
                            memset(&var, 0, sizeof(var));
                            continue;
                        }

#else   /* _ATL_PERSIST_QI_SUPPORT */
...
ATLINLINE ATLAPI AtlIPersistStreamInit_Save(LPSTREAM pStm, BOOL fClearDirty, ATL_PROPMAP_ENTRY* pMap, void* pThis, IUnknown* pUnk) {
        return AtlIPersistStreamInit_Save2(pStm, fClearDirty, pMap, pThis, pUnk, true);
    }
#endif //_ATL_DLL


#ifdef _ATL_DLL
ATLAPI AtlIPersistStreamInit_GetSizeMax(ATL_PROPMAP_ENTRY* pMap, void* pThis,
                                        IUnknown* pUnk, ULARGE_INTEGER* pSize);
#else
ATLINLINE ATLAPI AtlIPersistStreamInit_GetSizeMax2(ATL_PROPMAP_ENTRY* pMap, void* pThis,
                                         IUnknown* pUnk, ULARGE_INTEGER* pSize, bool fVersionCheck)
{

    if (!pMap || !pThis || !pUnk || !pSize) {
        return E_POINTER;
    }

    if (fVersionCheck) {
        pSize->QuadPart = sizeof(DWORD);
    } else {
        pSize->QuadPart = 0;
    }
    ...
    for (int i = 0; pMap[i].pclsidPropPage != NULL; i++)
    {
        if (pMap[i].piidDispatch && *pMap[i].piidDispatch == __uuidof(IChainPropMapFlagDummy)) {
            ...
            ULARGE_INTEGER s;
            hr = AtlIPersistStreamInit_GetSizeMax2(p, 
                                                  reinterpret_cast<void*>(pMap[i].dwOffsetData + (DWORD_PTR)pThis), 
                                                  pUnk, 
                                                  &s, false /* no ver check in chained bases(only in most derived) */);
            if (FAILED(hr)) {
                ATLTRACE2(atlTraceCOM, 0, _T("Failed to Chain Property Map in Stream GetSizeMax = %d\n"), i);
                return hr;
            }
            pSize->QuadPart += s.QuadPart;
            continue;
        }
        ...
        if (pMap[i].dwSizeData != 0)
        {
            ...
            case VT_CLSID: {
                ...
                // don't write the tag, we know the type and it breaks back compat
                // with existing persisted instances of the ocx
                pSize->QuadPart += sizeof(VARTYPE); 
#endif
                pSize->QuadPart += pMap[i].dwSizeData;
                continue;
            }
            // fall through and let CComVariant handle everything else.
            // i'm being pedantic and listing all the types we expect to handle
...
ATLINLINE ATLAPI AtlIPersistStreamInit_GetSizeMax(ATL_PROPMAP_ENTRY* pMap, void* pThis,
                                        IUnknown* pUnk, ULARGE_INTEGER* pSize) {
    return AtlIPersistStreamInit_GetSizeMax2(pMap, pThis, pUnk, pSize, true);
}
#endif //_ATL_DLL
...
ATLINLINE ATLAPI AtlIPersistPropertyBag_Load(LPPROPERTYBAG pPropBag, LPERRORLOG pErrorLog, ATL_PROPMAP_ENTRY* pMap, void* pThis, IUnknown* pUnk)
{
        ...
        for (int i = 0; pMap[i].pclsidPropPage != NULL; i++)
        {
                ...
                if (pMap[i].dwSizeData != 0)
                {
                        ...
                        // NOTE: this code assumes that var.vt is the same after the read
                        // as it was when it got passed in from the property map. i.e it assumes that the bag 
                        // implementation won't ever change the type which is an unsafe assumption.
                        // we need to do something if pmap[i].vt != var.vt
                        // possibly we should attempt a VariantChangeType to match the map and bail if that fails?
                        if (SUCCEEDED(hr))
                        {
                                ...
                                case VT_I2:
                                        *((short*)pData) = var.iVal;
                                        break;
                                case VT_UI2:
                                        *((unsigned short*)pData) = var.uiVal;
                                        break;
                                case VT_I4:
                                case VT_INT:
                                        *((long*)pData) = var.lVal;
                                ...
                                case VT_DISPATCH: {
                                    ...
                                    if (var.pdispVal != NULL)
                                        var.pdispVal->QueryInterface(__uuidof(IDispatch), (void **) pData);
                                    else
                                        *((BYTE*)pData) = NULL;
                                } break;
                                case VT_ARRAY | VT_UI1: {
                                ...
                                case VT_CLSID:
                                    if (var.vt == VT_BSTR) {
                                        hr = CLSIDFromString(var.bstrVal, reinterpret_cast<CLSID *>(pData));
                                        if (FAILED(hr)) {
                                            ATLTRACE2(atlTraceCOM, 2, _T("CLSIDFromString failed.  hr = %lx\n"), hr);
        ...
        for (int i = 0; pMap[i].pclsidPropPage != NULL; i++)
        {
                ...
                if (pMap[i].dwSizeData != 0)
                {
                        ...
                        case VT_I2:
                                ...
                                break;
                        case VT_UI2:
                                var.uiVal = *((unsigned short*)pData);
                                bTypeOK = true;
                                break;
                        case VT_INT:
                        case VT_I4:
                        ...
                        case VT_CLSID: {
                                ...
                                if (FAILED(hr)) {
                                    ATLTRACE2(atlTraceCOM, 2, _T("b.CopyTo failed.  hr = %lx\n"), hr);
                                    return hr;
                                }
                                bTypeOK = true;
                                break;
                        ...
                        if (var.vt == VT_BSTR_BLOB) {
                            // we're done with this variant
                            // so flip the variant tag so ::variantclear knows what to do 
                            // when it autodestructs
                            var.vt = VT_BSTR;  
                        }
                        continue;
                }
...
// for some weird reason IPersistStreamInit duplicates IPersistStream instead of inheriting
// but the impl should be the same

template <class T>
class ATL_NO_VTABLE IPersistStreamInitImpl : public IPersistStreamInit, public IPersistStream
{
public:
        // IPersist
        STDMETHOD(GetClassID)(CLSID *pClassID)
        ...
        STDMETHOD(GetSizeMax)(ULARGE_INTEGER FAR* pSize)
        {
                ATLTRACE2(atlTraceCOM, 0, _T("IPersistStreamInitImpl::GetSizeMax\n"));
                T* pT = static_cast<T*>(this);
                return pT->IPersistStreamInit_GetSizeMax(pSize, T::GetPropertyMap());
        }
        ...
        {
                ATL_LOCKT();
                HRESULT hr = AtlIPersistStreamInit_Save(pStm, fClearDirty, pMap, pT, pT->GetUnknown());
                if (SUCCEEDED(hr) && fClearDirty) {
                    pT->m_bRequiresSave = FALSE;
                }
                return hr;
        }
        HRESULT IPersistStreamInit_GetSizeMax(ULARGE_INTEGER* pSize, ATL_PROPMAP_ENTRY* pMap)
        {
                ATL_LOCKT();
                return AtlIPersistStreamInit_GetSizeMax(pMap, pT, pT->GetUnknown(), pSize);
        }
};
...
// NOTE: we should port property map chaining from persiststream & persistpropertybag to storage too.
// so far, i don't need it.  don't have time.  don't have test cases.
...
#define BEGIN_EXTERN_OBJECT_MAP(x) \
                            ...
                            static const _ATL_OBJMAP_ENTRY NULL_OBJECT_ENTRY = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL}; \
                            static const _ATL_OBJMAP_ENTRY * const _E##x [] = {
#define END_EXTERN_OBJECT_MAP()   &NULL_OBJECT_ENTRY }; 
#define DECLARE_EXTERN_OBJECT_ENTRY(classname) extern const _ATL_OBJMAP_ENTRY classname##OBJMAP_ENTRY;
#define EXTERN_OBJECT_ENTRY(classname) &##classname##OBJMAP_ENTRY,
#define DEFINE_EXTERN_OBJECT_ENTRY(clsid, classname) \
   extern const _ATL_OBJMAP_ENTRY classname##OBJMAP_ENTRY =    \
...
inline _ATL_OBJMAP_ENTRY* CreateObjectMapFromExternMap(const _ATL_OBJMAP_ENTRY* const * p) {
    int i;
...
public:
        ...
        {
#ifndef ATL_NO_TRUNCATE_RESOURCE_IDS
    		return AtlReportError(GetObjectCLSID(), nID, iid, hRes, hInst);
#else
	    	return AtlReportError2(GetObjectCLSID(), nID, iid, hRes, hInst, NULL);
#endif
    	}

	    static HRESULT WINAPI Error(UINT nID, const IID& iid,
		    HRESULT hRes, HINSTANCE hInst, va_list* Arguments)
	    {
		    return AtlReportError2(GetObjectCLSID(), nID, iid, hRes, hInst, Arguments);
	    }
...
public:
        ...
        HRESULT GetTypeInfo(UINT itinfo, LCID lcid, ITypeInfo** pptinfo)
        {
                ...
                {
#ifdef _ATL_DEBUG
                        OutputDebugStringA("CComTypeInfoHolder::GetTypeInfo returning DISP_E_BADINDEX because itinfo != 0\n");
#endif
...
#ifdef ATL_COM_OVERRIDE_TYPE_INFO_HOLDER_INVOKE
        ...
        HRESULT GetFuncInfoFromId(const IID& /*iid*/, DISPID dispidMember, LCID lcid, _ATL_FUNC_INFO& info)
        {
                ...
                for (int i=0; i<pFuncDesc->cParams; i++)
                {
                        ...
                        if (info.pVarTypes[i] == VT_PTR)
                                info.pVarTypes[i] = static_cast<VARTYPE>(pFuncDesc->lprgelemdescParam[i].tdesc.lptdesc->vt | VT_BYREF);
...
protected:
		...
		int nAllocCnt = nCnt;
		pPages->pElems = NULL;
	        pPages->pElems = (GUID*) CoTaskMemAlloc(sizeof(CLSID)*nAllocCnt);
