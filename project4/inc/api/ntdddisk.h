...
// the attributes on partitions
//

#define IOCTL_DISK_GET_PARTITION_ATTRIBUTES CTL_CODE(IOCTL_DISK_BASE, 0x003a, METHOD_BUFFERED, FILE_READ_ACCESS)
...
typedef struct _GET_PARTITION_ATTRIBUTES {
    ...
    //
    // Specifies the attributes that are
...
typedef struct _SET_PARTITION_ATTRIBUTES {
    ...
    // be applied to the partition
    //
    ULONGLONG Attributes;

    //
    // Indicates whether the change
    // needs to be undone on reboot
    //
    BOOLEAN RevertOnBoot;

    //
    // Indicates  whether  the attributes
    // apply to all  partitions  residing
    // on the  disk hosting the partition
    // Required if the disk layout is MBR
    //
    BOOLEAN ApplyToAllConnectedPartitions;
...
typedef enum _DISK_ONLINE_POLICY {

    DiskOnlinePolicyUnknown,
    DiskOnlinePolicyOnline,
    DiskOnlinePolicyOfflineShared,
    DiskOnlinePolicyOffline

} DISK_ONLINE_POLICY, *PDISK_ONLINE_POLICY;
...
typedef struct _DISK_SAN_SETTINGS {
    ...
    //
    DISK_ONLINE_POLICY OnlinePolicy;

} DISK_SAN_SETTINGS, *PDISK_SAN_SETTINGS;


//
// IOCTLs to query and toggle between
// the read-only and read-write modes
//

#define IOCTL_DISK_IS_ONLINE                CTL_CODE(IOCTL_DISK_BASE, 0x0082, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_DISK_ONLINE                   CTL_CODE(IOCTL_DISK_BASE, 0x0083, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)
#define IOCTL_DISK_OFFLINE                  CTL_CODE(IOCTL_DISK_BASE, 0x0084, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

//
// IOCTL_DISK_IS_ONLINE
//
// Input Buffer:
//     None
//
...
//
// Input Buffer:
//     Structure of type DISK_SET_SAN_MODE
//
// Output Buffer:
//     None
//

typedef struct _DISK_SET_SAN_MODE {
    ...
    //
    // Specifies an identifier to
    // associate with  the caller
    //
    GUID Owner;

} DISK_SET_SAN_MODE, *PDISK_SET_SAN_MODE;


//
// IOCTL to remove one  or all of the
// phantom entries from the SAN table
//

#define IOCTL_DISK_PURGE_SAN_TABLE          CTL_CODE(IOCTL_DISK_BASE, 0x0085, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

//
// IOCTL_DISK_PURGE_SAN_TABLE
//
// Input Buffer:
//     Structure of type DISK_PURGE_SAN_TABLE
//
// Output Buffer:
//     None
//

typedef struct _DISK_PURGE_SAN_TABLE {
    ...
    // field is a signature or guid
    //
    PARTITION_STYLE PartitionStyle;

    //
    // Specifies the signature or
    // guid that is  to be purged
    //
    union {
        ULONG Signature;
        GUID  Guid;
    ...
    //
    // Indicates whether all phantom
    // entries  should be purged out
    //
    BOOLEAN PurgeAll;
...
} DISK_PURGE_SAN_TABLE, *PDISK_PURGE_SAN_TABLE;
...
#define IOCTL_DISK_IS_CLUSTERED             CTL_CODE(IOCTL_DISK_BASE, 0x0086, METHOD_BUFFERED, FILE_ANY_ACCESS)
