...
#pragma warning(disable: 4616)	// warning number 'x' out of range, must be between '4001' and '4999'
...
public:
	...
	HRESULT GetIDsOfNames(REFIID /* riid */, __deref_in LPOLESTR* rgszNames, UINT cNames,
	...
	__checkReturn HRESULT LoadNameCache(ITypeInfo* pTypeInfo)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				FUNCDESC* pfd;
				// make sure at least init to zero for each iteration
				pMap[i].nLen = 0;
				pMap[i].id = 0;
...
public:
	...
	HRESULT InvokeFromFuncInfo(void (__stdcall T::*pEvent)(), _ATL_FUNC_INFO& info, DISPPARAMS* pdispparams, VARIANT* pvarResult)
	{
		...
		ATLASSERT(pdispparams->cNamedArgs <= (UINT)info.nParams);
		if (pdispparams->cNamedArgs > (UINT)info.nParams)
		{
			return E_FAIL;
		}
		// Pre-initialize so we can detect repeated or omitted arguments.
		for (nIndex; nIndex < pdispparams->cNamedArgs; nIndex++)
		{
			pVarArgs[nIndex] = NULL;
		}
		for (nIndex = 0; nIndex < pdispparams->cNamedArgs; nIndex++)
		{
			UINT nArgIndex = (UINT)pdispparams->rgdispidNamedArgs[nIndex];
			ATLASSERT(nArgIndex < pdispparams->cNamedArgs);
			if ((nArgIndex >= pdispparams->cNamedArgs) ||
				(nArgIndex >= _countof(rgVarArgs)))
			{
				return E_FAIL;
			}
			ATLASSERT(pVarArgs[nArgIndex] == NULL);
			if (pVarArgs[nArgIndex] != NULL)
			{
				return E_FAIL;
			}
			pVarArgs[nArgIndex] = &pdispparams->rgvarg[nIndex];
		}
#endif
...
public:
	...
	STDMETHOD(GetIDsOfNames)(REFIID riid, __deref_in LPOLESTR* rgszNames, UINT cNames,
		LCID lcid, DISPID* rgdispid)
...
protected:
	HRESULT GetPagesHelper(CAUUID* pPages, ATL_PROPMAP_ENTRY* pMap)
	{
		...
		int nAllocCnt = nCnt;
		pPages->pElems = NULL;
		pPages->pElems = (GUID*) ::ATL::AtlCoTaskMemCAlloc(nAllocCnt, static_cast<ULONG>(sizeof(CLSID)));
